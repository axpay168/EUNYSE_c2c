  ...
  <div>
    <main class="max-w-3xl mx-auto px-4 py-8">
      <section class="glass-panel rounded-[28px] p-5 md:p-6">
        ...
        <LoadingInlineSpinner v-if="loading" aria-label="提現詳情載入" />
        <template v-else>
        <div class="mt-4 flex items-start justify-between gap-4">
          <div><p class="text-[11px] uppercase tracking-[0.18em] text-on-surface-variant">提現明細</p><h1 class="font-headline text-2xl mt-1">提現詳情</h1></div>
        ...
        <div class="mt-6 space-y-3">
          ...
          <div class="rounded-2xl bg-surface-bright border border-outline-variant/20 p-4 flex items-center justify-between"><span class="text-on-surface-variant">提領金額</span><span class="font-headline text-xl">{{ item.amount || '-' }} {{ item.asset_code || '' }}</span></div>
          ...
          <div class="rounded-2xl bg-surface-bright border border-outline-variant/20 p-4 flex items-center justify-between"><span class="text-on-surface-variant">出金地址</span><span>{{ item.payout_address || '-' }}</span></div>
        </div>
        <div class="grid grid-cols-3 gap-3 mt-6 text-sm">
          ...
          <a class="rounded-2xl bg-surface-container-low px-4 py-4 text-center" href="#/pages/setting/withdrawUsdt">再次提現</a>
...
/**
 * EURNYSE - 提現詳情 (純 H5 Vue2 Options API)
